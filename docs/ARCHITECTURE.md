...
- **配置目录**：使用 `/etc/nginx/conf.d/job-assistant.conf`，不再走 `sites-available/sites-enabled`
- **访问前缀**：仅允许 `http://<公网IP>/job-application/` 与 `http://<公网IP>/job-assistant/`
- **根路径封禁**：直接访问 `http://<公网IP>/` 返回 404，不暴露服务入口
- **前缀转发**：Nginx 会把上述两个前缀重写为后端根路径 `/`
...
# 3. 配置 nginx
sudo cp deploy/nginx-site.conf /etc/nginx/conf.d/job-assistant.conf
...
部署完成后，对外入口只有：
- `http://<公网IP>/job-application/`
- `http://<公网IP>/job-assistant/`
